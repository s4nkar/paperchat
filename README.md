...
See [backend/docs/architecture.md](backend/docs/architecture.md) for the full system diagram and component breakdown.
...
Static retrieval metrics on 5 golden questions derived from a sentiment analysis research paper:
...
For the rationale behind this evaluation approach and the roadmap toward production-grade online evaluation (Ragas, LangSmith, LLM-as-judge), see [backend/docs/decisions.md](backend/docs/decisions.md#adr-11-static-golden-set-evaluation-over-online-evaluation).
...
CI/CD is handled via GitHub Actions — every push to `main` runs the real test suite first, then force-pushes to the HF Space git remote on success. See [.github/workflows/deploy.yml](.github/workflows/deploy.yml).
...
```bash
# Real logic tests (no API keys needed)
...
Key architectural decisions and their trade-offs are documented in [backend/docs/decisions.md](backend/docs/decisions.md) — covering hybrid retrieval, NDJSON streaming, single-container deployment, evaluation strategy, and more.
...
backend/
  ...
  scripts/        ablation.py
  docs/           architecture.md, decisions.md, ablation.md
  tests/
    mock/         mocked integration tests (documented contracts)
frontend/
  src/
    ...
    types.ts
.github/
  workflows/      deploy.yml (test → deploy to HF Spaces)
```
